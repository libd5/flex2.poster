...
class BaseSDTrainProcess(BaseTrainProcess):
    ...
    def setup_adapter(self):
        raise RuntimeError(
            "PosterGen minimal build supports LoRA + Canny only. Remove adapter config."
        )
        # t2i adapter
    ...
    def run(self):
        ...
        if not self.is_fine_tuning:
            if self.network_config is not None:
                ...
                if self.network_config.type.lower() in ('locon', 'lycoris', 'lokr', 'dora'):
                    raise ValueError(
                        f"PosterGen minimal build only supports network type 'lora', not {self.network_config.type!r}"
                    )
                ...
                if self.network_config.layer_offloading:
                    raise RuntimeError("PosterGen minimal build does not support layer_offloading")

            if self.embed_config is not None:
                raise RuntimeError("PosterGen minimal build does not support embedding training")
            ...
            if self.decorator_config is not None:
                raise RuntimeError("PosterGen minimal build does not support decorators")
