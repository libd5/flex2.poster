...
class Flex2(BaseModel):
    ...
    def condition_noisy_latents(self, latents: torch.Tensor, batch:'DataLoaderBatchDTO'):
        with torch.no_grad():
            ...
            def match_batch_size(tensor: torch.Tensor) -> torch.Tensor:
            ...
            if inpaint_tensor is not None and not do_dropout:
                ...
                inpainting_latent = match_batch_size(batch.latents)
            ...
            if do_dropout:
                # dropout with zeros
                control_latent = torch.zeros_like(match_batch_size(batch.latents))
            else:
                # we only have one control so we randomly pick from this list
                control_tensor = random.choice(control_tensor_list)
                control_tensor = match_batch_size(control_tensor)
