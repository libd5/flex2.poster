...
from toolkit.models.base_model import BlankNetwork
...
from toolkit.compat import adapter_is
...
class SDTrainer(BaseSDTrainProcess):
    ...
    def hook_before_train_loop(self):
        ...
        if self.train_config.diffusion_feature_extractor_path is not None:
            raise RuntimeError("PosterGen minimal build does not support diffusion feature extractors")
            vae = self.sd.vae
    ...
    def get_prior_prediction(
        ...
        if self.adapter is not None and (adapter_is(self.adapter, 'IPAdapter') or
                                         adapter_is(self.adapter, 'ReferenceAdapter') or
                                         (adapter_is(self.adapter, 'CustomAdapter'))
        ):
            can_disable_adapter = True
            was_adapter_active = self.adapter.is_active
            self.adapter.is_active = False

        if self.train_config.unload_text_encoder and self.adapter is not None and not adapter_is(self.adapter, 'CustomAdapter'):
        ...
        with torch.no_grad():
            ...
            if (self.adapter is not None and adapter_is(self.adapter, 'ClipVisionAdapter')) or self.embedding is not None:
            ...
            if self.adapter is not None and adapter_is(self.adapter, 'IPAdapter') and not self.sd.is_flux and not self.sd.is_lumina2:
    ...
    def train_single_accumulation(self, batch: DataLoaderBatchDTO):
        with torch.no_grad():
            ...
            if adapter_is(self.adapter, 'CustomAdapter'):
            ...
            if self.adapter and adapter_is(self.adapter, 'CustomAdapter'):
            ...
            if self.adapter is not None and adapter_is(self.adapter, 'IPAdapter') and not has_clip_image and has_adapter_img:
                raise ValueError(
        ...
        with self.timer('grad_setup'):
            ...
            if self.adapter and adapter_is(self.adapter, 'ClipVisionAdapter'):
                grad_on_text_encoder = True
        ...
        for noisy_latents, noise, timesteps, conditioned_prompts, imgs, adapter_images, clip_images, mask_multiplier, prompt_2 in zip(
            ...
            with (network):
                ...
                if self.adapter and adapter_is(self.adapter, 'CustomAdapter') and (has_clip_image or is_reg):
                ...
                if self.adapter and adapter_is(self.adapter, 'IPAdapter'):
                ...
                if self.adapter and adapter_is(self.adapter, 'ReferenceAdapter'):
                ...
                # do the custom adapter after the prior prediction
                if self.adapter and adapter_is(self.adapter, 'CustomAdapter') and (has_clip_image or self.adapter_config.type in ['llm_adapter', 'text_encoder']):
                    quad_count = random.randint(1, 4)
                    self.adapter.train()
                ...
                if self.adapter and adapter_is(self.adapter, 'CustomAdapter') and batch.extra_values is not None:
    ...
    def hook_train_loop(self, batch: Union[DataLoaderBatchDTO, List[DataLoaderBatchDTO]]):
        ...
        if self.adapter is not None and adapter_is(self.adapter, 'ClipVisionAdapter'):
