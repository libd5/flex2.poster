...
class BaseModel:
            ...
            with torch.no_grad():
                ...
                for i in tqdm(range(len(image_configs)), desc=f"Generating Samples", leave=False):
                    ...
                    if self.adapter is not None and gen_config.adapter_image_path is not None:
                        ...
                        if adapter_is(self.adapter, 'CustomAdapter') and self.adapter.control_lora is not None:
                        ...
                        if adapter_is(self.adapter, 'IPAdapter') or adapter_is(self.adapter, 'ClipVisionAdapter'):
                        ...
                        if adapter_is(self.adapter, 'ReferenceAdapter'):
                            # need -1 to 1
                    ...
                    if self.adapter is not None and adapter_is(self.adapter, 'CustomAdapter'):
                    ...
                    if self.adapter is not None and adapter_is(self.adapter, 'CustomAdapter') and validation_image is not None:
                    ...
                    if self.adapter is not None and adapter_is(self.adapter, 'IPAdapter') \
                            and gen_config.adapter_image_path is not None:
                    ...
                    if self.adapter is not None and adapter_is(self.adapter, 'CustomAdapter') and len(
                ...
                if self.adapter is not None and adapter_is(self.adapter, 'ReferenceAdapter'):
    ...
    def save_device_state(self):
        ...
        if self.adapter is not None:
            if adapter_is(self.adapter, 'IPAdapter'):
                ...
                adapter_device = self.adapter.device
            elif adapter_is(self.adapter, 'ClipVisionAdapter'):
                requires_grad = self.adapter.embedder.training
                adapter_device = self.adapter.device
            elif adapter_is(self.adapter, 'CustomAdapter'):
                requires_grad = self.adapter.training
                adapter_device = self.adapter.device
            elif adapter_is(self.adapter, 'ReferenceAdapter'):
                # todo update this!!
                requires_grad = True
                adapter_device = self.adapter.device
            else:
